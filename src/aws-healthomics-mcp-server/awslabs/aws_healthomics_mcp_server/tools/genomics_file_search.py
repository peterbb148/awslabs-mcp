...
def _normalize_field_default(value: Any, fallback: Any = None) -> Any:
    """Resolve pydantic Field defaults when tool functions are called directly.

    When the function is invoked outside FastMCP argument parsing (for example through
    a Lambda wrapper), optional arguments can arrive as `FieldInfo` objects.
    """
...
async def search_genomics_files(
    ...
    """
    try:
        # Normalize defaults when this function is called outside FastMCP argument parsing.
        file_type = _normalize_field_default(file_type, None)
        search_terms = _normalize_field_default(search_terms, [])
        max_results = _normalize_field_default(max_results, 100)
        include_associated_files = _normalize_field_default(include_associated_files, True)
        offset = _normalize_field_default(offset, 0)
        continuation_token = _normalize_field_default(continuation_token, None)
        enable_storage_pagination = _normalize_field_default(enable_storage_pagination, False)
        pagination_buffer_size = _normalize_field_default(pagination_buffer_size, 500)
        adhoc_s3_buckets = _normalize_field_default(adhoc_s3_buckets, None)
        ...
        if adhoc_s3_buckets is not None and not isinstance(adhoc_s3_buckets, list):
            return await handle_tool_error(
                ctx,
                ValueError('adhoc_s3_buckets must be a list of S3 bucket paths'),
                'Invalid adhoc buckets',
            )
